...
class ApolloLaneDataset(Dataset):
    ...
    def WIP__getitem__(self, idx):
        ...
        project_g2c, camera_k = self.get_camera_matrix(gt_cam_pitch, gt_cam_height)
        ...
        H_g2im, P_g2im, H_crop = self.transform_mats_impl(_label_cam_pitch, _label_cam_height)
        ...
        for i, lane in enumerate(gt_lanes):
            ...
            if _gt_laneline_category_org[i] > self.num_category:
                continue
            lane_ground = np.concatenate([lane, np.ones([lane.shape[0], 1])], axis=1).T
