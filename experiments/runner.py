...
class Runner:
    ...
    def train(self):
        ...
        for epoch in range(args.start_epoch, args.nepochs):
            ...
            for i, extra_dict in enumerate(train_loader):
                ...
                train_pbar.set_postfix(loss=training_meter['all'].avg)
            ...
            if meet_eval_freq or last_ep:
                ...
                if is_main_process():
                    write_val(valCSVFilename, lossjson2, epoch)
    ...
    def _log_training_loss(self, output, epoch, step, data_loader):
        ...
        for k, v in output.items():
            if 'loss' in k:
                loss = loss + v
                loss_json[k] = v.item() if isinstance(v, torch.Tensor) else v
    ...
    def log_eval_stats(self, eval_stats):
        ...
        if is_main_process():
            ...
            return {'f_measure': eval_stats[0],
                'recall': eval_stats[1],
                'precision': eval_stats[2],
                'category_accuracy': eval_stats[3],
                'x_error_close': eval_stats[4],
        ...
        if is_main_process():
            self.logger.info("===> Evaluation on validation set: \n"
                ...
                "laneline z error (far)  {:.8} m\n".format(eval_stats[0], eval_stats[1],
                                                            ...
                                                            eval_stats[6]))
            return {'f_measure': eval_stats[0], 'recall': eval_stats[1], 'precision': eval_stats[2],
                'x_error_close': eval_stats[3], 'x_error_far': eval_stats[4], 'z_error_close': eval_stats[5],
